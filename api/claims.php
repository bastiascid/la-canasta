...
if ($method === 'POST') {
    ...
    if (empty($name) || empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL) || empty($comments)) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Nombre, correo electrónico válido y detalles de la sugerencia son campos obligatorios.'
        ]);
        exit;
    }
    
    try {
        $stmt = $pdo->prepare("INSERT INTO claims (name, company, rut, phone, email, comuna, claim_type, invoice_number, comments, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Nuevo')");
        $stmt->execute([$name, $company, $rut, $phone, $email, $comuna, $claim_type, $invoice_number, $comments]);
        ...
        // Email to Admin
        $admin_subject = "=?UTF-8?B?" . base64_encode("[Nueva Sugerencia #$claim_id] $claim_type") . "?=";
        ...
        <html>
        <head>
            <title>Nueva Sugerencia Recibida</title>
        </head>
        <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
            <div style='max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;'>
                <h2 style='color: #5d4637; border-bottom: 2px solid #b83b1d; padding-bottom: 10px;'>Nueva Sugerencia Capturada (#$claim_id)</h2>
                <p>Se ha recibido una nueva sugerencia a través del sitio web:</p>
                <table style='width: 100%; border-collapse: collapse; margin-top: 15px;'>
                    ...
                    <tr><td style='padding: 8px 0; font-weight: bold;'>Tipo Sugerencia:</td><td>" . htmlspecialchars($claim_type) . "</td></tr>
        ...
        $user_subject = "=?UTF-8?B?" . base64_encode("Recibimos tu sugerencia #$claim_id - La Canasta") . "?=";
        ...
        <html>
        <head>
            <title>Recibimos tu Sugerencia</title>
        </head>
        <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
            <div style='max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;'>
                ...
                <p>Hemos recibido correctamente tu sugerencia, registrada con el número de seguimiento <strong>#$claim_id</strong>.</p>
                <p>Nuestro equipo lo revisará detalladamente para seguir mejorando. Nos pondremos en contacto contigo a la brevedad si es necesario.</p>
                <div style='background-color: #f9f9f9; padding: 15px; border-radius: 4px; margin-top: 20px;'>
                    <h4 style='margin-top: 0; color: #5d4637;'>Resumen de la Sugerencia:</h4>
                ...
                <p style='margin-top: 25px;'>Atentamente,<br><strong>Servicio al Cliente</strong><br>Comercializadora La Canasta</p>
            </div>
        </body>
        </html>
        ";
    ...
    try {
        ...
        if ($action === 'export') {
            ...
            if ($format === 'xls') {
                ...
                header('Content-Disposition: attachment; filename=sugerencias_canasta_' . date('Ymd_His') . '.xls');
                ...
                echo "ID\tNombre\tEmpresa\tRUT\tTeléfono\tEmail\tComuna\tTipo Sugerencia\tN° Factura/Guía\tDetalles\tEstado\tFecha Registro\n";
                foreach ($claims as $row) {
                    $comments = str_replace(["\r", "\n", "\t"], ' ', $row['comments']);
                    $comuna = isset($row['comuna']) ? $row['comuna'] : '';
                    echo "{$row['id']}\t{$row['name']}\t{$row['company']}\t{$row['rut']}\t{$row['phone']}\t{$row['email']}\t{$comuna}\t{$row['claim_type']}\t{$row['invoice_number']}\t{$comments}\t{$row['status']}\t{$row['created_at']}\n";
                }
            } else {
                header('Content-Type: text/csv; charset=utf-8');
                header('Content-Disposition: attachment; filename=sugerencias_canasta_' . date('Ymd_His') . '.csv');
                header('Pragma: no-cache');
                header('Expires: 0');
                
                echo "\xEF\xBB\xBF";
                $output = fopen('php://output', 'w');
                fputcsv($output, ['ID', 'Nombre', 'Empresa', 'RUT', 'Teléfono', 'Email', 'Comuna', 'Tipo Sugerencia', 'N° Factura/Guía', 'Detalles', 'Estado', 'Fecha Registro'], ';');
                foreach ($claims as $row) {
                    $comuna = isset($row['comuna']) ? $row['comuna'] : '';
                    fputcsv($output, [
                        $row['id'],
